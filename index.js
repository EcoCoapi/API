...
app.put("/ecole/:id", (req, res) => {
    ...
    con.query(
        `UPDATE Ecoles SET nom = '${nom}', ville= '${ville}', departement = '${departement}', region = '${region}', nbBus = '${nbBus}', nbPistecCycblable = '${nbPisteCyclable}', nbStationVelo = '${nbStationVelo}', type = '${type}' WHERE ( idEcole = '${id}');`, 
        function(error, result){
            if(error)console.log(error)
            else {
                console.log(result)
